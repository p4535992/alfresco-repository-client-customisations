...
function main()
{
   ...
   try
   {
      ...
      else if (url.templateArgs.type == "siteTags")
      {
    	  parent = classification.getRootCategories("cm:taggable")[0].parent;
    	  
          var site = siteService.getSite(argsSelectableType);
          if (site === null)
          {
             status.setCode(status.STATUS_NOT_FOUND, "Site " + argsSelectableType + " does not exist");
             return null;
          }
          
          var container = site.getContainer("documentLibrary");
          var scope = container.tagScope;
          if (scope == null)
          {
          ...
          var tags=scope.getTopTags(30);
          for each (var tag in tags)
          {
        	  results.push(
        	  {
        	    item: taggingService.getTag("workspace://SpacesStore", tag.getName()),
        	    selectable: true
        	  });
          }
      }
