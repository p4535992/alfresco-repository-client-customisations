 ...
 * 
 * Modified by Surevine to support searching by organisation name for user identities of the form 'uniqueid-orgname'
 * 
 * @author davidc
 * @author kevinr
 * @author paulg
        ...
        else
        {
            ...
            if (filter.length() != 0)
            {
                ...
                if (t.countTokens() == 1)
                {
                    ...
                    if (propIndex == -1)
                    {
                        // simple search: first name, last name, username starting with term, or username ending with "-term"
                ...
                else
                {
                    ...
                    while (t.hasMoreTokens())
                    {
                        ...
                        if (!propertySearch && term.indexOf(':') == -1)
                        {
                            if (nonFtsTokens == 1)
                            {
                                // simple search: first name, last name, username starting with term and username ending with "-term"
                                query.append("(firstName:\"");
                                query.append(term);
